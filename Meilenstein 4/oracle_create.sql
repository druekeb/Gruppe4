...
/
CREATE TABLE ARTIKEL_MARKUPS
(
  MARKUP_ID NUMBER NOT NULL,
  ARTIKEL_ID NUMBER NOT NULL,
  CONSTRAINT ARTIKEL_MARKUPS_PK PRIMARY KEY (MARKUP_ID,ARTIKEL_ID),
  CONSTRAINT ARTIKEL_MARKUPS_ARTIKEL_FK FOREIGN KEY (ARTIKEL_ID)
  REFERENCES ARTIKEL(ARTIKEL_ID),
  CONSTRAINT ARTIKEL_MARKUPS_MARKUP_FK FOREIGN KEY (MARKUP_ID)
...
/
CREATE TABLE INSTITUTE_MARKUPS
(
  MARKUP_ID NUMBER NOT NULL,
  INSTITUTS_ID NUMBER NOT NULL,
  FELDNAME VARCHAR(32),
  CONSTRAINT INSTITUTE_MUPS_PK PRIMARY KEY (MARKUP_ID),
  CONSTRAINT INSTITUTE_MUPS_MUPS_FK FOREIGN KEY (MARKUP_ID)
  REFERENCES MARKUPS(MARKUP_ID),
  CONSTRAINT INSTITUTE_MUPS_INSTITUTE_FK FOREIGN KEY (INSTITUTS_ID)
  REFERENCES INSTITUTE(INSTITUTS_ID),
  CONSTRAINT INSTITUTE_MUPS_CK 
  CHECK (upper(FELDNAME) IN ('NAME'))
 )
/
...
(
  ...
  CHECK (upper(FELDNAME) IN ('NAME','POSTADDRESSE','GEBAEUDETYP'))
 )
/
...
(
  ...
  CHECK (upper(FELDNAME) IN ('NAME','GEBAEUDE_ID','STOCKWERK','RAUMTYP'))
 )
/
...
(
  ...
  CHECK (upper(FELDNAME) IN ('NAME','GEBAEUDE_ID'))
 )
/
...
REFERENCES MITARBEITER(MITARBEITER_ID)
/
CREATE OR REPLACE TRIGGER  INSERT_GEB_MUPS_TR
  after insert on GEBAEUDE_MARKUPS               
  for each row  
begin   
    insert into MARKUPS(MARKUP_ID, TYP) values(:new.MARKUP_ID, 'GEBAEUDE');
end; 
/
CREATE OR REPLACE TRIGGER  INSERT_RAUM_MUPS_TR
  after insert on RAEUME_MARKUPS               
  for each row  
begin   
    insert into MARKUPS(MARKUP_ID, TYP) values(:new.MARKUP_ID, 'RAUM');
end; 
/
CREATE OR REPLACE TRIGGER  INSERT_FAK_MUPS_TR
  after insert on FAKULTAETEN_MARKUPS               
  for each row  
begin   
    insert into MARKUPS(MARKUP_ID, TYP) values(:new.MARKUP_ID, 'FAKULTAET');
end; 
/
CREATE OR REPLACE TRIGGER  INSERT_LEHRGEB_MUPS_TR
  after insert on LEHRGEBIETE_MARKUPS               
  for each row  
begin   
    insert into MARKUPS(MARKUP_ID, TYP) values(:new.MARKUP_ID, 'LEHRGEBIET');
end; 
/
CREATE OR REPLACE TRIGGER  INSERT_MITARB_MUPS_TR
  after insert on MITARBEITER_MARKUPS               
  for each row  
begin   
    insert into MARKUPS(MARKUP_ID, TYP) values(:new.MARKUP_ID, 'MITARBEITER');
end; 
/
CREATE OR REPLACE TRIGGER  INSERT_KURSE_MUPS_TR
  after insert on KURSE_MARKUPS               
  for each row  
begin   
    insert into MARKUPS(MARKUP_ID, TYP) values(:new.MARKUP_ID, 'KURS');
end; 
/
...
/
 CREATE SEQUENCE   "MARKUP_SEQ"  MINVALUE 1 MAXVALUE 999999999999999999999999999 INCREMENT BY 1 START WITH 1 CACHE 20 NOORDER NOCYCLE
/
...
CREATE OR REPLACE TRIGGER  "BI_GEBAEUDE_MARKUPS" 
  before insert on "GEBAEUDE_MARKUPS"
  for each row  
begin   
    select "MARKUP_SEQ".nextval into :NEW.MARKUP_ID from dual; 
end; 
/
ALTER TRIGGER  "BI_GEBAEUDE_MARKUPS" ENABLE
/
CREATE OR REPLACE TRIGGER  "BI_RAEUME_MARKUPS" 
  before insert on "RAEUME_MARKUPS"
  for each row  
begin   
    select "MARKUP_SEQ".nextval into :NEW.RAUM_ID from dual; 
end; 
/
ALTER TRIGGER  "BI_RAEUME_MARKUPS" ENABLE
/

CREATE OR REPLACE TRIGGER  "BI_MITARBEITER_MARKUPS" 
  before insert on "MITARBEITER_MARKUPS"
  for each row  
begin   
    select "MARKUP_SEQ".nextval into :NEW.MARKUP_ID from dual; 
end; 
/
ALTER TRIGGER  "BI_MITARBEITER_MARKUPS" ENABLE
/

CREATE OR REPLACE TRIGGER  "BI_KURSE_MARKUPS" 
  before insert on "KURSE_MARKUPS"
  for each row  
begin   
    select "MARKUP_SEQ".nextval into :NEW.MARKUP_ID from dual; 
end; 
/
ALTER TRIGGER  "BI_KURSE_MARKUPS" ENABLE
/
CREATE OR REPLACE TRIGGER  "BI_LEHRGEBIETE_MARKUPS" 
  before insert on "LEHRGEBIETE_MARKUPS"
  for each row  
begin   
    select "MARKUP_SEQ".nextval into :NEW.MARKUP_ID from dual; 
end; 
/
ALTER TRIGGER  "BI_LEHRGEBIETE_MARKUPS" ENABLE
/

CREATE OR REPLACE TRIGGER  "BI_INSTITUTE_MARKUPS" 
  before insert on "INSTITUTE_MARKUPS"
  for each row  
begin   
    select "MARKUP_SEQ".nextval into :NEW.MARKUP_ID from dual; 
end; 
/
ALTER TRIGGER  "BI_INSTITUTE_MARKUPS" ENABLE
/

CREATE OR REPLACE TRIGGER  "BI_FAKULTAET_MARKUPS" 
  before insert on "FAKULTAET_MARKUPS"
  for each row  
begin   
    select "MARKUP_SEQ".nextval into :NEW.MARKUP_ID from dual; 
end; 
/
ALTER TRIGGER  "BI_FAKULTAETEN_MARKUPS" ENABLE
/